package asm.org.apache.activemq.store.kahadb.disk.index;
import org.objectweb.asm.AnnotationVisitor;
import org.objectweb.asm.Attribute;
import org.objectweb.asm.ClassReader;
import org.objectweb.asm.ClassWriter;
import org.objectweb.asm.ConstantDynamic;
import org.objectweb.asm.FieldVisitor;
import org.objectweb.asm.Handle;
import org.objectweb.asm.Label;
import org.objectweb.asm.MethodVisitor;
import org.objectweb.asm.Opcodes;
import org.objectweb.asm.RecordComponentVisitor;
import org.objectweb.asm.Type;
import org.objectweb.asm.TypePath;
public class BTreeVisitorDump implements Opcodes {

public static byte[] dump () throws Exception {

ClassWriter classWriter = new ClassWriter(0);
FieldVisitor fieldVisitor;
RecordComponentVisitor recordComponentVisitor;
MethodVisitor methodVisitor;
AnnotationVisitor annotationVisitor0;

classWriter.visit(V1_8, ACC_PUBLIC | ACC_ABSTRACT | ACC_INTERFACE, "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "<Key:Ljava/lang/Object;Value:Ljava/lang/Object;>Ljava/lang/Object;", "java/lang/Object", null);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$LTEVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "LTEVisitor", ACC_PUBLIC | ACC_STATIC);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$LTVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "LTVisitor", ACC_PUBLIC | ACC_STATIC);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$GTEVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "GTEVisitor", ACC_PUBLIC | ACC_STATIC);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$GTVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "GTVisitor", ACC_PUBLIC | ACC_STATIC);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$BetweenVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "BetweenVisitor", ACC_PUBLIC | ACC_STATIC);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$AndVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "AndVisitor", ACC_PUBLIC | ACC_STATIC);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$OrVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "OrVisitor", ACC_PUBLIC | ACC_STATIC);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$PredicateVisitor", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "PredicateVisitor", ACC_PUBLIC | ACC_STATIC | ACC_ABSTRACT);

classWriter.visitInnerClass("org/apache/activemq/store/kahadb/disk/index/BTreeVisitor$Predicate", "org/apache/activemq/store/kahadb/disk/index/BTreeVisitor", "Predicate", ACC_PUBLIC | ACC_STATIC | ACC_ABSTRACT | ACC_INTERFACE);

{
methodVisitor = classWriter.visitMethod(ACC_PUBLIC | ACC_ABSTRACT, "isInterestedInKeysBetween", "(Ljava/lang/Object;Ljava/lang/Object;)Z", "(TKey;TKey;)Z", null);
methodVisitor.visitEnd();
}
{
methodVisitor = classWriter.visitMethod(ACC_PUBLIC | ACC_ABSTRACT, "visit", "(Ljava/util/List;Ljava/util/List;)V", "(Ljava/util/List<TKey;>;Ljava/util/List<TValue;>;)V", null);
methodVisitor.visitEnd();
}
classWriter.visitEnd();

return classWriter.toByteArray();
}
}
